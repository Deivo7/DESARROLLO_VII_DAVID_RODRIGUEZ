...
function generarReporteBiblioteca($biblioteca) {
    ...
    $reporte['libros_por_genero'] = array_reduce($biblioteca, function($acc, $libro) {
        if (!isset($acc[$libro['genero']])) {
            $acc[$libro['genero']] = 0;
        }
        $acc[$libro['genero']]++;
        return $acc;
    ...
    $reporte['autor_mas_libros'] = array_search(max($conteoAutores), $conteoAutores);
